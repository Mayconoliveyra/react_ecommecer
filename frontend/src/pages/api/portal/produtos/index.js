...
const getProdutoPortal = async ({ id, _page = 1, _limit = 20, _sort = "id", _order = "DESC", session }) => {
  const axios = await apiPortal(session);
  if (id) {
...
const saveProdutoPortal = async ({ data, id, session }) => {
  const axios = await apiPortal(session);
  if (id) {
